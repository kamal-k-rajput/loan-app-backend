...
router.put("/:loanId/approve", requireRole(ROLES.LENDER), validateBody(approveLoanSchema), approveLoanController);
router.put("/:loanId/reject", requireRole(ROLES.LENDER), validateBody(rejectLoanSchema), rejectLoanController);
router.post("/:loanId/disburse", requireRole(ROLES.LENDER), validateBody(disburseLoanSchema), disburseLoanController);
...
router.get("/:loanId/prepayment-calc", requireAuth, calculatePrepaymentController);
router.post("/:loanId/prepayment", requireAuth, validateBody(prepaymentSchema), processPrepaymentController);
...
import { moveToReviewSchema, assignLenderSchema } from "./loanAssignment.validators.js";

router.put("/:loanId/move-to-review", requireRole(ROLES.ADMIN), validateBody(moveToReviewSchema), moveLoanToReviewController);
router.put("/:loanId/assign-lender", requireRole(ROLES.ADMIN), validateBody(assignLenderSchema), assignLenderController);
