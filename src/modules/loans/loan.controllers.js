...
export async function applyLoanController(req, res, next) {
  try {
    ...
    const loan = await applyLoanService(db, session, req.body, req.user.dealerId);
    ...
    if (err.message === "LOAN_AMOUNT_OUT_OF_RANGE" || err.message === "TENURE_OUT_OF_RANGE") {
...
export async function updateLoanController(req, res, next) {
  try {
    ...
    const loan = await updateLoanService(db, session, req.params.loanId, req.body, req.user);
    return res.success(loan, "LOAN_UPDATED");
  } catch (err) {
    if (err.message === "LOAN_NOT_FOUND") {
      return res.fail(404, "LOAN_NOT_FOUND");
    }
    if (err.message === "LOAN_CANNOT_BE_UPDATED" || err.message === "CANNOT_UPDATE_OTHER_DEALER_LOAN") {
      return res.fail(400, err.message);
    }
    next(err);
  }
...
export async function cancelLoanController(req, res, next) {
  try {
    ...
    const ok = await cancelLoanService(db, session, req.params.loanId, req.user);
    ...
    if (err.message === "DISBURSED_LOAN_CANNOT_BE_CANCELLED" || err.message === "CANNOT_CANCEL_OTHER_DEALER_LOAN") {
      return res.fail(400, err.message);
    }
    next(err);
  }
...
export async function getLoansByStatusController(req, res, next) {
  try {
    ...
    const session = req.mongoSession;
    if (!req.user) return res.fail(401, "AUTH_REQUIRED");
    const loans = await getLoansByStatusService(db, session, req.params.status, req.user);
